    ...
    const normalizedCourses = useMemo(() => {
        ...
        return enrolledCourses
            .map((course: any, index: number) => {
                ...
                // Extract attemptId
                const attemptId = courseMeta?.attemptId || course?.attemptId || courseProgress?.attemptId;
        ...
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            >
                ...
                <CompleteProfileWidget
                    // Pass 0 if loading to avoid jumping
                    percentage={loading ? 0 : percentageValue}
                    onUpdatePress={handleUpdateProfile}
                    loading={loading && !profilePercentage} // Show skeleton if loading AND no percentage data
                />
                            ...
                            <>
                                {activeCourses.map((course) => {
                                    ...
                                    if (isSurvey) {
                                        ...
                                        // Calculate reattempt days (if available from API)
                                        const reattemptDays = course?.retakeDays || course?.raw?.reattemptDays || course?.raw?.daysUntilReattempt || 60;
                                        ...
                                        const isSurveyInProgress = course?.raw?.CourseProgress?.courseProgress === 'inProgress' ||
                                            course?.buttonText?.toLowerCase().includes('resume') ||
                                            course?.raw?.Courses?.attemptId;
                                        const surveyAttemptId = course?.attemptId || course?.raw?.Courses?.attemptId || course?.raw?.attemptId;
